...
TEST_F(DbTestSuit, TestGetByHandleFail) {
    ...
    EXPECT_TRUE(res);
    auto result = sync_wait(user_repo_.getByHandle("some_user"));
...
TEST_F(DbTestSuit, TestGetByIdFail) {
    ...
    EXPECT_TRUE(res);
    auto result = sync_wait(user_repo_.getById(100));
    EXPECT_FALSE(result.has_value());
}

TEST_F(DbTestSuit, TestGetAll) {
    /* When getAll called,
    it should return all users*/
    bool res1 = sync_wait(user_repo_.create("user1", "user1", "hash_idk"));
    bool res2 = sync_wait(user_repo_.create("user2", "user1", "hash_idk"));
    bool res3 = sync_wait(user_repo_.create("user3", "user1", "hash_idk"));
    EXPECT_TRUE(res1);
    EXPECT_TRUE(res2);
    EXPECT_TRUE(res3);
    auto users = sync_wait(user_repo_.getAll());
...
TEST_F(DbTestSuit, TestConcurrentCreateSameHandle) {
    /* When multiple threads attemp to create users
    with the same handle concurrently,
    exactly one should succeed and
    ...
    for (int i = 0; i < 10; i++) {
        threads.push_back(std::thread([&]() {
            bool res = sync_wait(user_repo_.create(
