...
TEST_F(UsersControllerTest, RegisterUserSuccess){
    ...
    create() will make new one and return true. Controller will return status code 201*/
    ...
    ASSERT_NE(resp, nullptr) << "Controller returned nullptr(just didn't return HttpResp)";
    ...
    EXPECT_EQ((*expected_response_json)["message"].asString(), "New user was successfully created");
}
...
TEST_F(UsersControllerTest, RegisterUserAlreadyExists){
    ...
    auto expected_response_json = resp->getJsonObject();
    ASSERT_NE(expected_response_json, nullptr) << "Response is not valid json";
    EXPECT_EQ((*expected_response_json)["message"].asString(), "User already exists");
}
...
TEST_F(UsersControllerTest, RegisterUserMissedFields){
    ...
    auto expected_response_json = resp->getJsonObject();
    ASSERT_NE(expected_response_json, nullptr) << "Response is not valid json";
    EXPECT_EQ((*expected_response_json)["message"].asString(), "Invalid JSON: couldn't find some fields. Missing fields: password");
}