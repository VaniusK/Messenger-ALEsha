...
При загрузке списка чатов нужно больше данных, чем просто `Chat`:
- Последнее сообщение (текст, время, автор)
- Количество непрочитанных сообщений
- Для директов: данные собеседника
...
Используем **cursor-based** пагинацию (`before_id` + `limit`), не offset.
- Offset плохо масштабируется на больших данных
- Cursor стабилен при добавлении новых сообщений
...
Комментарии реализованы как сообщения в discussion-чате.
При создании поста с комментариями:
1. Создаётся чат типа `discussion`
2. Создаётся "корневое" сообщение в этом чате
3. `discussion_message_id` в посте указывает на это сообщение
